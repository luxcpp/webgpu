...
//
// mpcvm_cggmp21.wgsl — CGGMP21 kernel entry for the wgpu/Dawn backend.
...
fn cggmp21_kind(k: u32) -> bool {
    return k == kKindCggmp21Keygen || k == kKindCggmp21Sign;
}

@compute @workgroup_size(1)
fn mpcvm_cggmp21_step(@builtin(global_invocation_id) gid: vec3<u32>) {
    if (gid.x != 0u) { return; }
