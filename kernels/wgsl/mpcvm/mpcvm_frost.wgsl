...
//
// mpcvm_frost.wgsl — FROST kernel entry for the wgpu/Dawn backend.
//
// Filters incoming CeremonyOps to FrostKeygen / FrostSign kinds and
// inserts begin entries into the ceremony arena. Subsequent state
// machine and root computation are handled by mpcvm_ceremony.wgsl
// and mpcvm_transition.wgsl.
...
fn frost_kind(k: u32) -> bool {
    return k == kKindFrostKeygen || k == kKindFrostSign;
}

@compute @workgroup_size(1)
fn mpcvm_frost_step(@builtin(global_invocation_id) gid: vec3<u32>) {
    if (gid.x != 0u) { return; }
